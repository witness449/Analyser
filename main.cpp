...
int main(int argc, char *argv[]) {
    ...
    std::ranges::for_each(analyseResults, [](const auto& res){
        std::println("{}{}::{}", res.f.filename, res.f.class_name.has_value()?"::"+res.f.class_name.value() :"", res.f.name);
    ...
    accumulator.RegisterAccumulator("count_of_lines", std::make_unique<analyser::metric_accumulator::metric_accumulator_impl::SumAverageAccumulator>()); 
    ...
    accumulator.RegisterAccumulator("count_parameters", std::make_unique<analyser::metric_accumulator::metric_accumulator_impl::AverageAccumulator>());
    ...
    std::ranges::for_each(splitByFiles, [&](const auto& splitted){
        ...
        auto& sumAvgCountLines=accumulator.GetFinalizedAccumulator<analyser::metric_accumulator::metric_accumulator_impl::SumAverageAccumulator>("count_of_lines");
        analyser::metric_accumulator::metric_accumulator_impl::SumAverageAccumulator::SumAverage result=sumAvgCountLines.Get();
    ...
    std::ranges::for_each(splitByClasses, [&](const auto& splitted){
        if(!splitted[0].f.class_name.has_value())return;
        accumulator.ResetAccumulators();
        analyser::AccumulateFunctionAnalysis(splitted, accumulator);
